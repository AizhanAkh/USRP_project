...
int background_functions(
  ...
  if (pba->has_efld == _TRUE_) {
    pvecback[pba->index_bg_rho_efld] = pba->Omega0_efld * pow(pba->H0,2) / pow(a_rel, 3.*(1.+pba->w0_efld));
...
int background_init(
  ...
  if (pba->has_efld == _TRUE_) {
    class_test(pba->w0_efld>=1./3.,
	       pba->error_message,
	       "Your choice for w_efld=%g is suspicious, since it is bigger than -1/3 there cannot be radiation domination at early times\n",
	       w0_efld);
